...
public class OvenCsvSourceImpl implements CsvSource<Oven> {
    private final String resourceName;

    public OvenCsvSourceImpl(String resourceName) {
        this.resourceName = resourceName;
    }

    @Override
    public List<Oven> readAll() {
        ...
        try (InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(resourceName);
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length >= 3) {
                    res.add(new Oven(parts[0].trim(), parts[1].trim(), Double.parseDouble(parts[2].trim())));
                }
            }
        } catch (Exception e) {
